...
الإصدار 4.11.0 تطبيق واجهة موحد يعمل بملفات HTML/CSS/JavaScript ويستخدم Supabase للهوية وقاعدة البيانات وRPC وRLS. المعمارية انتقالية: ما زال `app-core.js` يحتوي على منطق قديم كبير، بينما تنتقل المجالات تدريجيًا إلى خدمات ووحدات تحت `src/`.
...
تحتفظ الوحدة بواجهة `main` الحالية ونقطة الدخول الجذرية، لكن مصدر البيانات الوحيد للمسابقات والفئات والمشاركات هو Supabase عبر خدمات المجال. يبقى `show-office.js` Controller الـDOM الوحيد ولا توجد نسخة ثانية من CRUD داخل `app-core.js`. لا يقرأ النسخ الاحتياطي من حالة الواجهة أو `localStorage`، ويبقى مفتاح التخزين القديم دون حذف لاسترداد بيانات النسخة السابقة عند الحاجة.

يسجل منسق الوحدة موفر JSON غير متزامن يقرأ جميع المسابقات والفئات وسجلات المشاركات مباشرة من Supabase بتصفح Keyset. تستخدم الفئات مفتاح المسابقة المحمول، وتستخدم Entries مراجع UUID محمولة للفارس والخيل والإسطبل مع مفتاح البطولة والفئة بدل IDs المحلية. فشل أي موفر مسجل يلغي إنشاء النسخة كاملة ولا يُنزّل ملف جزئي. تمرر النواة الصفوف المنقاة إلى RPC واحدة؛ تتحقق PostgreSQL من دفعة Show Office الكاملة ثم تستعيدها ذريًا داخل معاملة واحدة. أما الاستعادة العامة للجداول القديمة المتعددة فتبقى Best-effort وليست معاملة واحدة، ولذلك قد تكتمل جزئيًا.
