-- Compatibility rollback for v4.27.0 (Horse Lease booking type).
-- Restores cce_public_submit_booking() to its pre-lease form (it rejects
-- 'lease' with "Unsupported booking request type" again — this is what
-- actually stops new lease requests, independent of the constraints below),
-- deactivates the lease_request service row, and drops the lease_* columns
-- on horses. Emergency use only — any negotiated lease terms recorded on
-- horses are lost.
--
-- Deliberately NOT done: deleting the lease_request service row or
-- re-tightening the request_type check constraints on
-- public_booking_services / booking_requests. Once any lease request has
-- been submitted, a booking_requests row references lease_request (FK) and
-- carries request_type='lease' — deleting the service row would violate
-- that FK, and re-tightening either check constraint would fail immediately
-- against that existing row. Deactivating (not deleting) the service row
-- keeps historical lease bookings intact and avoids both failure modes; the
-- constraints stay as-is (harmlessly still allowing 'lease' as a stored
-- value) since the reverted function is what actually blocks new lease
-- submissions.

begin;

alter table public.horses
  drop column if exists lease_active,
  drop column if exists lease_customer_name,
  drop column if exists lease_cpr,
  drop column if exists lease_address,
  drop column if exists lease_mobile,
  drop column if exists lease_monthly_price,
  drop column if exists lease_farrier_share,
  drop column if exists lease_farrier_weeks,
  drop column if exists lease_farrier_name,
  drop column if exists lease_trainer_name,
  drop column if exists lease_start_date;

update public.public_booking_services set active=false, updated_at=now() where code='lease_request';
