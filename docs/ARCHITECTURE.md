...
الإصدار 4.9.1 تطبيق واجهة موحد يعمل بملفات HTML/CSS/JavaScript ويستخدم Supabase للهوية وقاعدة البيانات وRPC وRLS. المعمارية انتقالية: ما زال `app-core.js` يحتوي على منطق قديم كبير، بينما تنتقل المجالات تدريجيًا إلى خدمات ووحدات تحت `src/`.
...
- `src/modules/show-office/competition-service.js`: التحقق والمثابرة والنسخ والاستعادة الخاصة بالمسابقات دون DOM.
- `src/services/backup-runtime.js`: تنسيق النسخ والاستعادة المتوافق مع الجداول القديمة وموفري الوحدات.

## وحدة Show Office

تحتفظ الوحدة بواجهة `main` الحالية ونقطة الدخول الجذرية، لكن مصدر البيانات الوحيد للمسابقات هو Supabase عبر `competition-service.js`. لا يقرأ Controller سجلات المسابقات من حالة الواجهة أو `localStorage`، ويبقى مفتاح التخزين القديم دون حذف لاسترداد بيانات النسخة السابقة عند الحاجة. لا توجد نسخة ثانية من منطق CRUD داخل `app-core.js`.

يسجل مجال المسابقات موفر JSON غير متزامن يقرأ جميع السجلات مباشرة من Supabase بتصفح Keyset، وموفر Restore يملك التحقق من `modules.showOffice`. فشل أي موفر مسجل يلغي إنشاء النسخة كاملة ولا يُنزّل ملف جزئي. تمرر النواة الصفوف المنقاة إلى RPC واحدة؛ تتحقق PostgreSQL من دفعة Show Office كاملة ثم تنفذها ذريًا داخل معاملة واحدة. أما الاستعادة العامة للجداول القديمة المتعددة فتبقى Best-effort وليست معاملة واحدة، ولذلك قد تكتمل جزئيًا.

لا تنشئ Sprint 1 جداول Classes أو Entries؛ يعرض Dashboard قيمتهما صفرًا حتى إضافة مصادرهما في السبرنتات المخصصة.
