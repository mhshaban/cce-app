-- CCE StableOS v4.15.0 — Accumulator with Joker scoring format
-- Reuses the v4.14.0 fence-tap workflow (show_office_entry_fences, the same
-- Clear/Knockdown/Refusal grid) but interprets it as points-per-fence instead
-- of faults, with a configurable Joker fence that doubles the total when
-- cleared. Classes stay on Table A (scoring_format='table_a') by default.
...
begin
  if to_regclass('public.show_office_classes') is null
    ...
    raise exception 'Show Office v4.14.0 must be applied before Accumulator with Joker';
  end if;
end $$;

alter table public.show_office_classes
  add column if not exists scoring_format text not null default 'table_a',
  add column if not exists joker_fence_number smallint;

alter table public.show_office_classes drop constraint if exists show_office_classes_scoring_format_check;
alter table public.show_office_classes add constraint show_office_classes_scoring_format_check
  check (scoring_format in ('table_a','accumulator_joker'));
alter table public.show_office_classes drop constraint if exists show_office_classes_accumulator_requires_fences_check;
alter table public.show_office_classes add constraint show_office_classes_accumulator_requires_fences_check
  check (scoring_format<>'accumulator_joker' or fence_count is not null);
alter table public.show_office_classes drop constraint if exists show_office_classes_joker_fence_check;
alter table public.show_office_classes add constraint show_office_classes_joker_fence_check
  check (joker_fence_number is null or (fence_count is not null and joker_fence_number between 1 and fence_count));

alter table public.show_office_entry_rounds add column if not exists points numeric(10,2);

alter table public.show_office_entry_rounds drop constraint if exists show_office_entry_rounds_complete_check;
alter table public.show_office_entry_rounds add constraint show_office_entry_rounds_complete_check check (
  eliminated or retired or did_not_start
  or (time_ms is not null and (faults is not null or points is not null))
);
alter table public.show_office_entry_rounds drop constraint if exists show_office_entry_rounds_points_check;
alter table public.show_office_entry_rounds add constraint show_office_entry_rounds_points_check
  check (points is null or (points>=0 and points<=999999.99));
alter table public.show_office_entry_rounds drop constraint if exists show_office_entry_rounds_faults_points_exclusive_check;
alter table public.show_office_entry_rounds add constraint show_office_entry_rounds_faults_points_exclusive_check
  check (faults is null or points is null);

-- Confirms a fence-scored round. For scoring_format='table_a' this is unchanged
-- from v4.14.0. For 'accumulator_joker' the total is the sum of each clear
-- fence's point value (its fence number; untouched fences are clear by
-- default), doubled if the configured Joker fence is clear. Elimination still
-- uses the same class-configured refusal threshold either way.
...
-- Judge Panel/Live Results: carries the class scoring format, Joker fence
-- number, and ranks Accumulator classes by points descending then time
-- ascending instead of faults ascending. Non-Accumulator classes are
-- unaffected.
...
-- Class restore now also accepts the optional scoring format and Joker fence
-- number; absent fields keep the table defaults (table_a / null) so backups
-- made before v4.15.0 restore unchanged.
...
begin
  ...
  loop
    ...
    if v_row?'scoring_format' and jsonb_typeof(v_row->'scoring_format')<>'null'
       and coalesce(v_row->>'scoring_format','table_a') not in ('table_a','accumulator_joker') then
      raise exception 'Show Office backup class % has invalid scoring_format',v_index using errcode='22023';
    end if;
    v_fence_count:=nullif(v_row->>'fence_count','')::integer;
    if coalesce(v_row->>'scoring_format','table_a')='accumulator_joker' and v_fence_count is null then
      raise exception 'Show Office backup class % requires fence_count for accumulator_joker',v_index using errcode='22023';
    end if;
    if v_row?'joker_fence_number' and jsonb_typeof(v_row->'joker_fence_number')<>'null' and (
      (v_row->>'joker_fence_number')::numeric<>trunc((v_row->>'joker_fence_number')::numeric)
      or (v_row->>'joker_fence_number')::numeric<1
      or v_fence_count is null or (v_row->>'joker_fence_number')::numeric>v_fence_count
    ) then
      raise exception 'Show Office backup class % has invalid joker_fence_number',v_index using errcode='22023';
    end if;
  end loop;
  ...
  loop
    ...
    insert into public.show_office_classes(
      ...
      fence_count,knockdown_fault_value,refusal_fault_value,refusals_before_elimination,
      scoring_format,joker_fence_number
    ) values(
      v_competition_id,
      ...
      coalesce(nullif(v_row->>'scoring_format',''),'table_a'),
      nullif(v_row->>'joker_fence_number','')::smallint
    )
...
comment on column public.show_office_classes.scoring_format is
  'table_a (default, faults then time) or accumulator_joker (points then time, Joker fence doubles the total).';
comment on column public.show_office_classes.joker_fence_number is
  'Fence number that doubles the total score when cleared, for accumulator_joker classes only.';
comment on column public.show_office_entry_rounds.points is
  'Accumulator with Joker total points. Null for table_a rounds and for eliminated/retired/DNS entries.';
