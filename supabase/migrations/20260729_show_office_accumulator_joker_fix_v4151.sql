-- CCE StableOS v4.15.1 — Hotfix: correct Joker doubling in Accumulator with Joker
-- v4.15.0 incorrectly doubled the entire round total when the Joker fence was
-- cleared. The Joker doubles only its own fence value. Example with an 8-fence
-- course, Joker on fence 8, all fences clear: 1+2+3+4+5+6+7+(8*2) = 44, not
-- (1+2+...+8)*2 = 72.
...
begin
  if to_regclass('public.show_office_classes') is null
    ...
    raise exception 'Show Office v4.15.0 must be applied before this fix';
  end if;
end $$;
...
begin
  ...
  if v_scoring_format='accumulator_joker' then
    -- Each clear fence scores its own number; the Joker fence scores double
    -- its own number when clear. Knocking/refusing any fence, including the
    -- Joker, scores 0 for that fence only — no other penalty.
    select coalesce(sum(
      case
