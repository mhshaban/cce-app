...
الإصدار 4.10.0 تطبيق واجهة موحد يعمل بملفات HTML/CSS/JavaScript ويستخدم Supabase للهوية وقاعدة البيانات وRPC وRLS. المعمارية انتقالية: ما زال `app-core.js` يحتوي على منطق قديم كبير، بينما تنتقل المجالات تدريجيًا إلى خدمات ووحدات تحت `src/`.
...
تحتفظ الوحدة بواجهة `main` الحالية ونقطة الدخول الجذرية، لكن مصدر البيانات الوحيد للمسابقات والفئات هو Supabase عبر خدمتي المجال. يبقى `show-office.js` Controller الـDOM الوحيد ولا توجد نسخة ثانية من CRUD داخل `app-core.js`. لا يقرأ النسخ الاحتياطي من حالة الواجهة أو `localStorage`، ويبقى مفتاح التخزين القديم دون حذف لاسترداد بيانات النسخة السابقة عند الحاجة.

يسجل منسق الوحدة موفر JSON غير متزامن يقرأ جميع المسابقات والفئات مباشرة من Supabase بتصفح Keyset، ثم يحول علاقة الفئة إلى مفتاح محمول هو اسم المسابقة الموحّد وتاريخها بدل الاعتماد على IDs بين البيئات. فشل أي موفر مسجل يلغي إنشاء النسخة كاملة ولا يُنزّل ملف جزئي. تمرر النواة الصفوف المنقاة إلى RPC واحدة؛ تتحقق PostgreSQL من دفعة Show Office الكاملة ثم تستعيد المسابقات والفئات ذريًا داخل معاملة واحدة. أما الاستعادة العامة للجداول القديمة المتعددة فتبقى Best-effort وليست معاملة واحدة، ولذلك قد تكتمل جزئيًا.

يجلب Dashboard عدد الفئات مباشرة من PostgreSQL دون تحميل جميع صفوفها. تحمل كل فئة `class_number` نصيًا للرموز مثل `1A` و`sort_order` رقميًا للترتيب التشغيلي. لا توجد Entries بعد؛ يبقى إجماليها صفرًا حتى Sprint 3.
