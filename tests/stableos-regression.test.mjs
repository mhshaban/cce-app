...
test('all app assets use the v4.17.3 cache key',()=>{
  const html=read('index.html');
  assert.ok(!html.includes('20260714-465'));
  assert.ok(!html.includes('20260731-4172'));
  assert.ok((html.match(/20260731-4173/g)||[]).length>=20);
  assert.match(read('app-bootstrap.js'),/stableos-20260731-4173/);
  assert.match(read('app-core.js'),/sw\.js\?v=20260731-4173/);
  assert.equal(read('VERSION.txt').trim(),'4.17.3');
});
